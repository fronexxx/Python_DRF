...
class ChatConsumer(GenericAsyncAPIConsumer):
    ...
    def get_last_five_messages(self):
        # res = self.room.messages.annotate(pk=F('user__pk'), name=F('user__profile__name')).values('pk', 'text', 'name').order_by('-id')[:5]
        res =  MessageModel.objects.filter(
            Q(room=self.room) | (Q(room__is_private=True) & Q(room__users__in=[self.scope['user']]))
        ).annotate(pk=F('user__pk'), name=F('user__profile__name')).values('pk', 'text', 'name').order_by('-id')[:5]
